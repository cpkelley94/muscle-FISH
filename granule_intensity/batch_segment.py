...
for i, line in enumerate(file_list):
    p_img, outdir, gene1, gene2, t_spot1, t_spot2 = line.replace('\r','').replace('\n','').split('\t')
    img_name = os.path.splitext(os.path.basename(p_img))[0]

    # cmd = 'module load python/3.6.5\n'
    cmd = 'python segment_spots.py "' + p_img + '" ' + outdir + ' ' + gene1
    if not gene2 == '.':
        cmd += ' ' + gene2
    if not t_spot1 == '.':
        cmd += ' -1 ' + t_spot1
    ...
    sh_paths.append(['seg' + outdir[0] + '_' + str(i).zfill(2), sh_path])
...
if should_run:
    ...
    for name, bp in sh_paths:
        cmd = 'slurmify-run ' + bp + ' -n ' + name + ' -m 16 -t 2 --burst'
