...
if should_plot:
    su.animate_zstacks([img_dapi, np.clip(img_rna, a_min=0, a_max=np.amax(img_rna)/5.)], titles=['DAPI', 'FISH, gain=5'], gif_name=os.path.join(outdir, 'anim', img_name+'_channels^'+gene+'.gif'))
...
img_rna_average = su.normalize_image(np.mean(np.array(imgs_rna), axis=0))
...
if should_plot:
    su.animate_zstacks([np.clip(img_rna, a_min=0, a_max=np.amax(img_rna)/5.), img_fiber_only], titles=['FISH, gain=5', 'fiber prediction'], gif_name=os.path.join(outdir, 'anim', img_name+'_fiber^'+gene+'.gif'))
...
if should_plot:
    su.animate_zstacks([region_nuc, region_peri, region_cyt, region_cyt + 2*region_peri + 3*region_nuc], titles=['nuclear', 'perinuclear', 'sarcoplasmic', 'all compartments'], cmaps=['binary_r', 'binary_r', 'binary_r', 'gnuplot2'], gif_name=os.path.join(outdir, 'anim', img_name+'_regions^'+gene+'.gif'))
