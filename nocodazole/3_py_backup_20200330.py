...
# conditional imports:
# czifile (CZI image format)
# bioformats, javabridge (OME-TIFF image format)
...
if p_czi.lower().endswith('.czi'):
...
elif any([p_czi.lower().endswith(ext) for ext in ['.ome.tiff', '.ome.tif', '.ometif', '.ometiff']]):
    ...
    for z in count():
        try:
            slice = bioformats.load_image(p_czi, z=z)
            slices.append(slice)
    ...
    # split channels
    img_dapi = scope_utils.normalize_image(img[:, :, :, -1].transpose(1,2,0))
    imgs_rna = [scope_utils.normalize_image(img[:, :, :, i].transpose(1,2,0)) for i in range(len(genes))]

    # look for metadata .XML file with same filename
    p_meta = os.path.splitext(p_czi)[0] + '.xml'
    try:
        meta = ElementTree.parse(p_meta)
    except IOError:
...
region_nuc_labeled = region_nuc_labeled - 1  # shift so bg is -1 and nuclei are 0 to n_nuc
nuc_meshes = []
centroids = []
for i in range(n_nuc):
    nuc = np.where(region_nuc_labeled == i, 1, 0)
    # nuc_shell = morphology.binary_dilation(nuc) - nuc
    # nuc_fade = nuc_shell + 2.*nuc
    # verts, faces, _, _ = measure.marching_cubes_lewiner(nuc_fade, 1)
    ...
    try:
        ...
        # glitch and I'm not sure what causes it
        # skip this nucleus
