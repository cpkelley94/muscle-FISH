...
elif any([p_img.lower().endswith(ext) for ext in ['.ome.tiff', '.ome.tif', '.ometif', '.ometiff']]):
...
if should_plot:
    scope_utils.animate_zstacks(imgs_rna + [img_dapi], vmax=[0.2]*len(imgs_rna) + [1], gif_name=os.path.join(outdir, 'anim', img_name+'_channels.gif'))
