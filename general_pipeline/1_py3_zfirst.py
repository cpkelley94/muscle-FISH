...
if fish_channel:
    try:
        img_rna = img[tracks.index(int(fish_channel)),:,:,:]
...
img_fiber_only = mf.threshold_fiber(img_rna, t_fiber=t_fiber, z_first=True, verbose=True)
...
nuclei_labeled = mf.threshold_nuclei(img_dapi, t_dapi=t_dapi, fiber_mask=img_fiber_only, z_first=True, verbose=True)
...
for n in range(10):  # ~ 0.5 um in x, y
    for z in range(region_nuc.shape[0]):
...
for n in range(40):  # ~ 2 um in x, y
    for z in range(img_nuc_dilated.shape[0]):
...
img_rna_corr = mf.fix_bleaching(img_rna, mask=img_fiber_only, draw=True, z_first=True, imgprefix=img_name)

print('Finding FISH spots...')
spots_masked, spot_data = mf.find_spots_snrfilter(img_rna_corr, sigma=2, snr=t_snr, t_spot=0.018, mask=img_fiber_only, z_first=True, imgprefix=img_name)
